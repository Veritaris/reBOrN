...
impl RebornCache {
    ...
    pub fn save_to_disk(&self) {
        ...
        let cache_string = ron::ser::to_string_pretty(
            &self,
            ron::ser::PrettyConfig::default(),
        ).unwrap();
        file.write(cache_string.as_bytes()).expect("unable to save default cache");
    }
}
...
pub fn mappings_exists(mc_version: &str, channel: &str, mappings_version: &str) -> bool {
    ...
    let save_path = store_dir.join("mappings").join(mc_version).join(channel).join(mappings_version);
...
pub fn ensure_versions_json(file_hash: Option<String>) -> Result<String, Error> {
    ...
    if !(&versions_json_path.exists() & hash_matches) {
        ...
        println!(
            "{}",
            if !hash_matches {
                format!("file hash does not match cached hash: '{} != {}'", file_hash.unwrap_or("".to_string()), current_file_hash)
            } else {
        ...
        return if let Err(err) = download_file(versions_json_path.as_path(), &versions_json_link) {
            match err.status() {
                Some(StatusCode::NOT_FOUND) => println!("unable to find resource {versions_json_link}"),
            ...
            Ok(sha256::try_digest(&versions_json_path).unwrap())
        };
    }
...
pub fn read_versions_json() -> LinkedHashMap<String, HashMap<String, Vec<String>>> {
    ...
    let hm = serde_json::from_reader::<BufReader<std::fs::File>, HashMap<String, HashMap<String, Vec<String>>>>(reader).unwrap();

    let mut sorted: Vec<(String, HashMap<String, Vec<String>>)> = hm.into_iter()
        .map(|(k, v)| {
            let new_key = k.split(".").map(|e| format!("{:0>2}", e)).collect::<Vec<String>>().join(".");
            (new_key, v)
        })
        .collect();

    sorted.sort_by_key(|(k, v)| {
        k.clone()
    });

    sorted = sorted.into_iter()
        .map(|(k, v)| {
            let new_key = k.split(".").map(|e| e.trim_start_matches("0").to_string()).collect::<Vec<String>>().join(".");
            (new_key, v)
        })
        .collect();
