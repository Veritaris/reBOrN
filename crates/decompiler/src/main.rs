...
fn main() {
    ...
    let file_path = std::path::Path::new("crates/decompiler/compiledJavaClasses/уmd.class");
...
fn write_methods(classfile: &ClassFile, buf: &mut Vec<String>, indent: Option<u8>) {
    ...
    for method in classfile.methods.iter() {
        let mut method_name = classfile.get_string_from_cpool(method.name_index);
        if method_name == CLASS_INIT_METHOD_NAME {
            let line_indent = TAB.repeat(indent as usize);
            let method_code = format!("{line_indent}static {{\n\n{line_indent}}}");
        ...
        if let JVMSignature::MethodSignature {
            params_spec,
            mut return_spec,
        } = method_signature
        {
            let method_params_signature = params_spec
                .iter()
                .map(|it| it.to_string())
                .collect::<Vec<String>>()
                .join(", ");
            ...
            if method_name == CONSTRUCTOR_METHOD_NAME {
                return_spec = ValueType::Object { r#type: String::new() };
                if let ConstantPoolTag::Class { name_index, .. } = classfile.get_from_cpool(classfile.this_class) {
                    method_name = classfile.get_string_from_cpool(name_index);
                }
            }
            for attr in method.attributes.iter() {
                ...
                {
                    for code_attr in attributes.iter() {
                        if let Attribute::LocalVariableTable {
                            attribute_name_index,
                            attribute_length,
                        ...
                        {
                            for one_local_variable in local_variable_table {
                                if let Some(JVMSignature::FieldSignature { spec }) = parse_jvm_descriptor(
                                    &classfile.get_string_from_cpool(one_local_variable.descriptor_index),
                                ) {
                                    println!(
                                        "{} {}",
                                        spec,
                                        classfile.get_string_from_cpool(one_local_variable.name_index)
                                    );
                                }
                            }
                        }
            ...
            let method_definition = vec![
                access_flags.to_string(),
                return_spec.to_string(),
                format!("{}({})", method_name, method_params_signature),
            ];
            let mut method_code = String::new();
            for one_method_code_part in method_definition {
                if one_method_code_part.is_empty() {
                    continue;
                }
                method_code += " ";
                method_code += &one_method_code_part;
            }
            let line_indent = TAB.repeat(indent as usize);
            method_code = line_indent.to_string() + method_code.trim() + " {}";
            buf.push(method_code);
        }
    }
}
    ...
    for field in classfile.fields.iter() {
        ...
        for attr in field.attributes.iter() {
            if let Attribute::ConstantValue {
                ...
                && let JVMSignature::FieldSignature { spec } = &field_descriptor
            {
                match spec {
                    ValueType::Primitive { .. } => {}
